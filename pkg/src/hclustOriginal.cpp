...
using namespace std;
void hclustOriginal::compute_phi() {
    int index[_K];
    for (int k=0;k<_K;k++)
        index[k] = 0;
    for (int k=0;k<_K;k++) {
...
void hclustOriginal::Init(double *Data, int *RuptVect) {
...
hclustOriginal::hclustOriginal(int n, int nbsegments, int nbclusters) {
    _lengthx = n ;
    _K       = nbsegments;
    _P       = nbclusters;
    _D       = new double *[_K-1];
...
// distance
double hclustOriginal::distance(double nA, double mA, double vA, double nB, double mB, double vB) {
    double ybar    = (nA*mA+nB*mB)/(nA+nB);
    double varpool =  (  nB*vB + nA*vA + nB*(mB-ybar)*(mB-ybar) + nA*(mA-ybar)*(mA-ybar) ) / (nB+nA);
    double res     = (nA+nB)*varpool -nA*vA-nB*vB;
    return(res);
}
